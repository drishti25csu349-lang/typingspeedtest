...
int main() {
    ...
    printf("\nSelect Test Type:\n");
    printf("1. Word Count-based Test (25/50/100 words)\n2. Custom Text by User\n");
    printf("Enter choice: ");
    int choice;
    ...
    if (choice == 1) {
        ...
        printf("Enter choice: ");
        int wc;
        if (scanf("%d", &wc) != 1) return 0;
        getchar();

        switch (level) {
            case 1:
                if (wc == 1) sample = easy25;
                else if (wc == 2) sample = easy50;
                else sample = easy100;
                break;
            case 2:
                if (wc == 1) sample = medium25;
                else if (wc == 2) sample = medium50;
                else sample = medium100;
                break;
            case 3:
                if (wc == 1) sample = hard25;
                else if (wc == 2) sample = hard50;
                else sample = hard100;
                break;
        }

        printf("\nYou selected a %d-word %s difficulty test.\n", 
               countWords(sample), 
        ...
        printf("\nEnter your own custom text for the test:\n");
        if (!fgets(custom, sizeof(custom), stdin)) return 0;
        custom[strcspn(custom, "\n")] = '\0';
        sample = custom;
        printf("\nCustom test selected (%d words).\n", countWords(sample));
    } 
    else {
    ...
    while (difftime(time(NULL), start) < timerSeconds) {
        ...
        printf("\rTime left: %2d seconds | Type here: ", timeLeft);
        fflush(stdout);
    ...
    float wpm = ((float)wordsTyped / timeTaken) * 60.0f;
    ...
    printf("Accuracy          : %.2f%%\n", accuracy);
    printf("===========================\n");
    printf("Timer up! Great job - keep practicing to improve your typing speed.\n");
